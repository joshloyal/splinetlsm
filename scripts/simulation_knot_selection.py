...
def simulation(seed, n_nodes=100, n_time_points=100, ls_type='gp', nu=0.5, density=0.2): 
    ...
    models, res = select_knots(
        base_model, Y, time_points, X,
        scale_factors=[0.5] + list(range(1, 8)),
        n_jobs=1)
    best_id = np.argmin(res['waic']) 
    ...
    for k, model in enumerate(models):
        ...
        data.append({
            ...
            'proc_corr_select': proc_corr_all,
            ...
            'k': k+1,
            'spline_dimension': res['spline_dimension'][k],
            'is_best': k == best_id
        })
    data = pd.DataFrame(data)

    out_file = f'result_{seed}.csv'
    dir_base = 'output_knot_selection'
