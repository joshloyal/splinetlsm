...
def simulation(seed, n_nodes=100, n_time_points=100, ls_type='gp', density=0.2): 
    seed = int(seed)
    n_nodes = int(n_nodes)
    ...
    Y, time_points, X, probas, U, coefs, intercept, z = synthetic_network_mixture(
        n_nodes=n_nodes, n_time_points=n_time_points, n_features=4,
        ls_type=ls_type, include_covariates=True, length_scale=0.2,
        tau=0.5, sigma=0.5, density=density, random_state=seed)
    y_true = dynamic_adjacency_to_vec(Y)

    model = SplineDynamicLSM(
        n_features=2, n_segments='auto', alpha=0.95, init_type='usvt', 
        random_state=4)
    ...
    U_pred, _ = longitudinal_procrustes_rotation(U[..., :2], model.U_)
    U_rmse = np.sqrt(np.mean((U[..., :2] - U_pred) ** 2))
    
    
    # pad estimated U matrix with zeros
    U_padded = np.concatenate([model.U_, np.zeros((model.U_.shape[0], model.U_.shape[1], 2))], axis=2)
    U_pred_all, _ = longitudinal_procrustes_rotation(U, U_padded)
    U_rmse_all = np.sqrt(np.mean((U - U_pred_all)** 2))
    
    # dimension selectio
    d_max = 2
    U_select = U[..., :d_max]
    U_pred, _ = longitudinal_procrustes_rotation(
        U_select, model.U_[..., :d_max])
    U_rmse_select = np.sqrt(np.mean((U_select - U_pred) ** 2))
    ...
    proc_corr_select = 0.
    for t in range(n_time_points):
        proc_corr += np.sqrt(1 - procrustes(U[t, :,  :2], model.U_[t, :, :2])[-1]) / n_time_points
        proc_corr_all += np.sqrt(1 - procrustes(U[t], U_padded[t])[-1]) / n_time_points
        proc_corr_select += np.sqrt(1 - procrustes(U_select[t], model.U_[t, :, :d_max])[-1]) / n_time_points
    ...
    data = {
        ...
        'n_features_total': model.n_features
    }
    data1 = pd.DataFrame(data, index=[0])
    
    # fit model with d = 6 as a comparison
    model = SplineDynamicLSM(
    ...
    # compare smoothed latent positions with a single procrustes transform
    U_pred, _ = longitudinal_procrustes_rotation(U, model.U_[..., :4])
    U_rmse = np.sqrt(np.mean((U - U_pred) ** 2))
    
    # pad true U matrix with zeros
    U_padded = np.concatenate([U, np.zeros((U.shape[0], U.shape[1], 2))], axis=2)
    U_pred_all, _ = longitudinal_procrustes_rotation(U_padded, model.U_)
    U_rmse_all = np.sqrt(np.mean((U_padded - U_pred_all)** 2))
     
    # dimension selection 
    U_padded_select = U_padded[..., :model.n_features_]
    U_pred, _ = longitudinal_procrustes_rotation(
        U_padded_select, model.U_[..., :model.n_features_])
    ...
    proc_corr_select = 0.
    for t in range(n_time_points):
        proc_corr += np.sqrt(1 - procrustes(U[t], model.U_[t, :, :4])[-1]) / n_time_points
        proc_corr_all += np.sqrt(1 - procrustes(U_padded[t], model.U_[t])[-1]) / n_time_points
        proc_corr_select += np.sqrt(1 - procrustes(U_padded_select[t], model.U_[t, :, :model.n_features_])[-1]) / n_time_points
    ...
    dir_base = 'output_underspecified_dim'
    dir_name = os.path.join(dir_base, 'output', f"gp_n{n_nodes}_T{n_time_points}_d{density}")
...
for density in [0.1, 0.2, 0.3]:
    for i in range(n_reps):
        simulation(seed=i, n_nodes=200, n_time_points=100, density=density)
